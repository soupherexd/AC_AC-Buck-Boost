...
//       EPWM8_Init();
   ...
   //PieCtrlRegs.PIEIER3.bit.INTx1 = 1;  // 启用 PIE 组3中断1（ePWM1）
    ...
    for (for_count = 0;for_count<sin_len;for_count++)sinx1[for_count]=0;
    for (for_count = 0;for_count<sin_len;for_count++)sinx2[for_count]=0;
...
__interrupt void adc_isr(void)
{
    U_in = (AdcResult.ADCRESULT0-2050.7)/36.237;             //A0
    U_out = (AdcResult.ADCRESULT5-2053.7)/36.758;            //A3
    I_L = (AdcResult.ADCRESULT6-2087.9)/325.51;              //B0
    ...
    {
        if (Duty_Cycle <= 0.03)Duty_Cycle = 0.03;               //大小限制
...
{                                                   //广义二阶积分SOGI
    ...
    last_err = err;
    if (fabs(result) <0.05)result = 0;

    W1 -= result;
    if (W1 >= 115 * pi) W1 = 115 * pi;          //361
    if (W1 <= 85 * pi) W1 = 85 * pi;            //267
    W1_pi = W1/2/pi;
...
void KEY_Control(int key)
{
    ...
    if (U_REF > 32)U_REF = 32;
    if (U_REF < 1)U_REF = 1;

    if (I_REF > 2)I_REF = 2;
    if (I_REF < 0.3)I_REF = 0.3;
}
