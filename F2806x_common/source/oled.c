...
void OLED_Init(void) {
    ...
    /* 上电后等待至少100ms让OLED芯片稳定 */
    DELAY_US(100000);

    /* === I2C总线释放预处理：发送9个时钟脉冲释放SDA === */
    I2C_ReleaseBus();
    ...
    OLED_WriteCommand(0x40);  //--set start line address (0)

    OLED_WriteCommand(0xb0);  //--set page address (page 0)

    OLED_WriteCommand(0x81);  //--set contrast control register
    OLED_WriteCommand(0xcf);  // 调节亮度，0X00-0XFF (SSD1315推荐0x7F~0xCF)

    OLED_WriteCommand(0xa1);  //--set segment re-map 127 to 0   a0:0 to seg127
    OLED_WriteCommand(0xa6);  //--set normal display (non-inverted)
    ...
    OLED_WriteCommand(0x80);  //--set divide ratio (default: 8:1)

    /* === SSD1315 充电泵使能（最关键！无此命令面板无驱动电压）=== */
    OLED_WriteCommand(0x8d);  //--charge pump setting
    OLED_WriteCommand(0x14);  //--enable charge pump (0x10=disable, 0x14=enable)

    OLED_WriteCommand(0xd9);  //--set pre-charge period
    OLED_WriteCommand(0x22);  //--SSD1315: 1时钟相位1 + 2时钟相位2 (外部VCC推荐0x22)

    OLED_WriteCommand(0xda);  //--set com pins hardware configuration (SSD1315)
    OLED_WriteCommand(0x12);  //--0x02: sequential, 0x12: sequential+remap (SSD1315推荐0x12)

    OLED_WriteCommand(0xdb);  //--set vcomh deselect level
    OLED_WriteCommand(0x20);  //--~0.77xVCC (推荐值)

    OLED_WriteCommand(0xaf);  //--turn on oled panel

    DELAY_US(10000);  // 等待显示稳定后再清屏
    OLED_Clean();
}
